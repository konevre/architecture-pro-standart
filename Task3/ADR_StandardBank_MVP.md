...
| NFR2 | **Производительность API:** Задержка ответа (Response Time) < 300 мс (95 перцентиль). Сервис ставок должен выдерживать 1000 RPS. |
| NFR3 | **Безопасность:** Шифрование трафика (TLS 1.3) для всех каналов; маскирование ПДн. |
| NFR4 | **Надежность АБС:** Прямые синхронные запросы из ИБ в БД АБС запрещены (0 RPS). Используется асинхронный обмен. |
| NFR5 | **Надежность SMS:** Процент доставки SMS > 99.9% (SLA). Ретраи на уровне шлюза. |
